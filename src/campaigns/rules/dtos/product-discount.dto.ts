...
import { IsBoolean, IsEnum, IsNotEmpty, ValidatorConstraint, ValidatorConstraintInterface, ValidationArguments, Validate } from "class-validator";
...
@ApiProperty({
    ...
    description: 'Array of product segments with discount rules. Each item must have: segmentName, percentage, minOrderValue, maxDiscount'
})
@IsNotEmpty({ message: 'segments field cannot be empty. Provide at least one product segment with discount details.' })
